...
export async function queryRagStream(c: Context) {
  return withSpan("rag.query.stream", { "http.route": "/rag/query/stream" }, async (span) => {
    ...
    span.setAttribute("rag.query.length", query.length);
    ...
    span.setAttribute("rag.state_code", effectiveState);
    ...
    return streamSSE(c, async (stream) => {
      ...
      try {
        ...
        span.setAttribute("rag.rejected", rejected);
        span.setAttribute("rag.answer.length", fullAnswer.length);
